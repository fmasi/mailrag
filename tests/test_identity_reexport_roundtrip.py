...
class TestReexportIdentityRoundTrip(unittest.TestCase):
    def setUp(self):
        base = _assemble(_HEADERS, _BODY, preamble=b"From - Wed Mar 27 15:27:20 2024\n188035\t\n")
    ...
    def test_reordered_headers_unchanged(self):
        reordered = [_HEADERS[3], _HEADERS[1], _HEADERS[4], _HEADERS[0], _HEADERS[2], _HEADERS[5]]
        self._assert_same_identity(_assemble(reordered, _BODY, preamble=b"From - x\n188035\t\n"))

    def test_extra_x_headers_unchanged(self):
        extra = _HEADERS + [b"X-Mailer: Some Re-Export Tool 9.0", b"X-Spam-Score: 0.1"]
        self._assert_same_identity(_assemble(extra, _BODY, preamble=b"From - x\n188035\t\n"))

    def test_trailing_body_whitespace_unchanged(self):
        body_ws = b"Hi Bob,   \n\nHere are the notes from today.\t\n\nThanks,\nAlice\n"
        self._assert_same_identity(_assemble(_HEADERS, body_ws, preamble=b"From - x\n188035\t\n"))
