...
from src.pipeline.judge import run, select_suspects
...
class TestJudgeRun(unittest.TestCase):
    def test_judges_only_suspects(self):
        ...
        with (
            mock.patch("src.pipeline.judge.os.path.exists", return_value=True),
            ...
            mock.patch(
                "src.pipeline.judge.run_pass", return_value={"done": 1, "cached": 0, "error": 0}
            ) as rp,
        ):
            counts = run(prof, model="m", scan_json="/tmp/s.json", min_score=0.6, progress=False)
