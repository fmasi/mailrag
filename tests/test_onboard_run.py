...
class TestRunOnboard(unittest.TestCase):
    def test_sequences_stages_and_returns_report(self):
        ...
        judgments = {
            ...
            "c": {"is_noise": False, "confidence": 0.1, "summary": "",
                  "reason": "llm_error: boom"},
        }
        with mock.patch.object(onboard, "_require_qdrant"), \
             ...
             mock.patch("src.llm.onboard_pass.generate_thread_judgments",
                        return_value=judgments), \
             mock.patch("src.indexing.contextual_index.build_contextual_index",
                        return_value=_BuildResult()) as build, \
             mock.patch.object(onboard, "write_manifest", return_value="/m.json"):
            report = onboard.run_onboard(
                "/some/dir", collection="mailrag-x", chunk_size=256,
                embedder="EMB", validate=False)
    ...
    def test_all_noise_raises(self):
        emails = [_Email("a")]
        judgments = {"a": {"is_noise": True, "confidence": 0.99, "summary": "",
                           "reason": ""}}
        with mock.patch.object(onboard, "_require_qdrant"), \
             mock.patch.object(onboard, "load_eml_dir", return_value=emails), \
             mock.patch.object(onboard, "assign_subject_fallback_thread_ids"), \
            ...
            with self.assertRaises(ValueError):
                onboard.run_onboard("/d", collection="c", chunk_size=256,
                                    embedder="EMB", validate=False)
