...
class TestCollect(unittest.TestCase):
    def test_from_judge_cache(self):
        ...
        cache = mock.Mock()
        cache.iter_noise.return_value = [
            _row("h1", 0.9, "newsletter"),
    ...
    def test_from_tag(self):
        ...
        with (
            mock.patch(
                "src.pipeline.prune.resolve_index_files",
...
class TestRun(unittest.TestCase):
    ...
    def test_confirm_false_writes_nothing(self):
        ...
        with (
            mock.patch("src.pipeline.prune.collect", return_value=(["h1"], ["preview"])),
    ...
    def test_confirm_true_appends(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            prof = self._prof(bl)
            with mock.patch("src.pipeline.prune.collect", return_value=(["h1", "h2"], ["p"])):
    ...
    def test_empty_drop_set_no_ops(self):
        prof = self._prof("/tmp/bl.txt")
        with (
            mock.patch("src.pipeline.prune.collect", return_value=([], [])),
            mock.patch("src.pipeline.prune.append_to_blacklist") as app,
        ):
