...
class TestExploreRun(unittest.TestCase):
    def _profile(self):
        prof = SimpleNamespace(
            collection="c", qdrant_url="http://localhost:6333",
            selection_rules=[], chunk_size=512)
    ...
    def test_uses_qdrant_when_collection_exists(self):
        ...
        with mock.patch("src.pipeline.explore.resolve_index_files",
                        return_value=(["/root/a.eml"], [])), \
             ...
             mock.patch("src.pipeline.explore.read_thread_vectors",
                        return_value=tvecs) as rtv, \
    ...
    def test_force_fresh_skips_qdrant_even_if_collection_exists(self):
        ...
        with mock.patch("src.pipeline.explore.resolve_index_files",
             ...
             mock.patch("src.pipeline.explore.BgeM3Embedder", return_value=embedder), \
             tempfile.TemporaryDirectory() as d:
            report = run(prof, json_path=os.path.join(d, "e.json"),
                         seed=11, force_fresh=True)
        rtv.assert_not_called()
        embedder.encode.assert_called_once()
        self.assertEqual(report.vector_source, "fresh")
