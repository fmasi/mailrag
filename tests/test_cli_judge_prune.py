...
def _prof(**kw):
    base = dict(
        rubric="personal",
        calibration={"rubric": "personal", "passed": True},
        blacklist=None,
        pass2_cache="/tmp/c.db",
    )
    base.update(kw)
...
class TestJudgeVerb(unittest.TestCase):
    def test_judge_routes_when_calibrated(self):
        with (
            mock.patch("src.cli.CorpusProfile.load", return_value=_prof()),
            mock.patch("src.cli.judge_stage.run", return_value={"done": 3, "suspects": 3}) as run,
        ):
            rc = cli.main(["judge", "--profile", "p.json", "--model", "m", "--min-score", "0.7"])
    ...
    def test_judge_uncalibrated_exits_2(self):
        with mock.patch("src.cli.CorpusProfile.load", return_value=_prof(calibration=None)):
...
class TestPruneVerb(unittest.TestCase):
    def test_prune_dry_run_without_yes(self):
        ...
        with (
            ...
            mock.patch(
                "src.cli.prune_stage.collect", return_value=(["h1", "h2"], ["0.9 newsletter"])
            ),
    ...
    def test_prune_writes_with_yes(self):
        ...
        with (
            mock.patch("src.cli.CorpusProfile.load", return_value=prof),
            mock.patch("src.cli.prune_stage.collect", return_value=(["h1"], ["0.9 x"])),
