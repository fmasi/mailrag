...
from src.onboard import OnboardReport, write_manifest, latest_manifest_collection


def _report(coll, validated=True):
    return OnboardReport(collection=coll, kept=10, noise_dropped=2, llm_failures=0,
                         chunks=12, chunk_size=512,
                         coverage_at3=0.8 if validated else None,
                         n_queries=5 if validated else 0, validated=validated)
