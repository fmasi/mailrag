...
def _reply(is_noise, summary):
    return json.dumps({"is_noise": is_noise, "confidence": 0.9,
                       "summary": summary, "reason": "r"})
...
class TestGenerateThreadJudgments(unittest.TestCase):
    ...
    def test_keeps_noise_judgment_and_summary(self):
        ...
        with mock.patch("src.llm.onboard_pass.default_model", return_value="M"), \
             ...
             mock.patch("src.llm.onboard_pass.chat",
                        side_effect=[_reply(False, "sum1"), _reply(True, "")]):
    ...
    def test_resumes_from_cache_without_calling_llm(self):
        ...
        with mock.patch("src.llm.onboard_pass.default_model", return_value="M"), \
             mock.patch("src.llm.onboard_pass.make_client", return_value="C"), \
             mock.patch("src.llm.onboard_pass.chat",
                        return_value=_reply(False, "first")) as chat1:
