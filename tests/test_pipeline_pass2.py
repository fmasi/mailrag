...
class TestPass2Stage(unittest.TestCase):
    def test_sweeps_selection_with_workers(self):
        ...
        prof = CorpusProfile(
            root="/r", selection_rules=[{"type": "prefix", "value": "a/"}], pass2_cache="/tmp/p.db"
        )
        with (
            mock.patch("src.pipeline.pass2.resolve_index_files", return_value=(["/r/a/x.eml"], [])),
            mock.patch("src.pipeline.pass2.Pass2Cache"),
            mock.patch("src.pipeline.pass2.llm_client") as cl,
            mock.patch(
                "src.pipeline.pass2.run_pass", return_value={"done": 1, "cached": 0, "error": 0}
            ) as rp,
        ):
    ...
    def test_summarize_uses_profile_rubric(self):
        ...
        with (
            ...
            mock.patch("src.pipeline.pass2.summary.parse_response", return_value=mock.MagicMock()),
