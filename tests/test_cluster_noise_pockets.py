...
from src.cluster.noise_pockets import (
    ThreadMeta, ClusterReport, default_k, cluster_threads,
)


def _meta(tid, n_emails, dominant, top_share, n_senders, tag_fraction, subjects, paths):
    return ThreadMeta(thread_id=tid, n_emails=n_emails, dominant_sender=dominant,
                      top_sender_share=top_share, n_senders=n_senders,
                      tag_fraction=tag_fraction, sample_subjects=subjects, paths=paths)
...
def _noisy_and_clean(dim=8):
    ...
    for i in range(20):
        ...
        metas.append(_meta(f"noise{i}", 1, "no-reply@bulk.com", 1.0, 1, 1.0,
                           ["Your weekly digest"], [f"noise{i}.eml"]))
    for i in range(20):
        v = rng.normal(scale=1.0, size=dim)
        v[0] = -1.0
        vecs.append(v)
        metas.append(_meta(f"real{i}", 3, f"alice{i}@work.com", 0.4, 3, 0.0,
                           ["Re: project plan"], [f"real{i}.eml"]))
...
class TestNoisePockets(unittest.TestCase):
    ...
    def test_baseline_and_json_shape(self):
        ...
        d = report.to_json_dict(profile="p.json", collection="c",
                                vector_source="fresh")
        ...
        cl0 = d["clusters"][0]
        for key in ("id", "size", "n_emails", "score", "components",
                    "top_senders", "n_senders", "sample_subjects", "members"):
