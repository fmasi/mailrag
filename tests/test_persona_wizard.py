...
class TestWizard(unittest.TestCase):
    ...
    def test_llm_all_calibrate_gate_proceed(self):
        ...
        bh, load, rec, prof = self._patch(calls)
        q = FakeQ(
            selects=["llm-all", "proceed to the LLM pass"], texts=["mymodel"], confirms=[True]
        )
        with bh, load, rec, mock.patch("src.llm.calibration.format_report", return_value="BUCKETS"):
            rc = run_wizard("p.json", questionary=q, console=FakeConsole())
        self.assertEqual(rc, 0)
    ...
    def test_calibrate_gate_abort_stops_before_summarize(self):
        ...
        with bh, load, rec, mock.patch("src.llm.calibration.format_report", return_value="B"):
            rc = run_wizard("p.json", questionary=q, console=FakeConsole())
    ...
    def test_summarize_confirm_no_stops(self):
        calls = []
        bh, load, rec, prof = self._patch(calls)
        q = FakeQ(selects=["llm-all", "proceed to the LLM pass"], texts=["m"], confirms=[False])
        with bh, load, rec, mock.patch("src.llm.calibration.format_report", return_value="B"):
            rc = run_wizard("p.json", questionary=q, console=FakeConsole())
        self.assertEqual(rc, 1)
        self.assertNotIn("summarize", calls)
    ...
    def test_llm_verify_runs_judge_and_prune(self):
        ...
        q = FakeQ(
            selects=["llm-verify", "proceed to the LLM pass"], texts=["mymodel"], confirms=[True]
        )
        with bh, load, rec, mock.patch("src.llm.calibration.format_report", return_value="B"):
            rc = run_wizard("p.json", questionary=q, console=FakeConsole())
        self.assertEqual(rc, 0)
        self.assertEqual(
            calls, ["scope", "measure", "scan", "calibrate", "judge", "prune", "summarize", "index"]
        )
