...
class TestGetQdrantClient(unittest.TestCase):
    ...
    def test_api_key_from_env(self):
        with patch("qdrant_client.QdrantClient") as QClient, \
             patch.dict(os.environ,
                        {"QDRANT_URL": "http://h:6333", "QDRANT_API_KEY": "secret"}):
            get_qdrant_client()
    ...
    def test_empty_api_key_becomes_none(self):
        with patch("qdrant_client.QdrantClient") as QClient, \
             patch.dict(os.environ,
                        {"QDRANT_URL": "http://h:6333", "QDRANT_API_KEY": ""}):
            get_qdrant_client()
    ...
    def test_explicit_api_key_overrides_env(self):
        with patch("qdrant_client.QdrantClient") as QClient, \
             patch.dict(os.environ,
                        {"QDRANT_URL": "http://h:6333", "QDRANT_API_KEY": "env-key"}):
    ...
    def test_explicit_prefer_grpc_overrides_env(self):
        with patch("qdrant_client.QdrantClient") as QClient, \
             patch.dict(os.environ,
                        {"QDRANT_URL": "http://h:6333", "QDRANT_PREFER_GRPC": "true"}):
