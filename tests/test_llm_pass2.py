...
class TestRunPass(unittest.TestCase):
    def setUp(self):
        ...
        self.f1 = os.path.join(self.dir.name, "a.eml"); _write(self.f1, b"one")
        self.f2 = os.path.join(self.dir.name, "b.eml"); _write(self.f2, b"two")
    ...
    def test_limit_stops_early(self):
        c = pass2.run_pass([self.f1, self.f2], self.cache, self._load,
                           self._summarize, model="m", limit=1)
    ...
    def test_progress_true_returns_same_counts(self):
        c = pass2.run_pass([self.f1, self.f2], self.cache, self._load,
                           self._summarize, model="m", progress=True)
    ...
    def test_process_file_stores_resilient_identity(self):
        ...
        def load(path):
            return {"sender": "a", "subject": "s", "date": "z", "body": "b",
                    "message_id": "<M@x>"}
...
class TestSampleFiles(unittest.TestCase):
    ...
    def test_different_seed_differs(self):
        paths = [str(i) for i in range(100)]
        self.assertNotEqual(pass2.sample_files(paths, 10, seed=1),
                            pass2.sample_files(paths, 10, seed=2))
