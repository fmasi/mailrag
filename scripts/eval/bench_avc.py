...
QFILE = sys.argv[1] if len(sys.argv) > 1 else os.environ.get("QUERIES", "eval/out/queries_360.jsonl")
HYBRID = os.environ.get("HYBRID_COL", "work-rag-ctx-threadaware")
E5 = os.environ.get("E5_COL", "work-rag-e5")
QD = os.environ["QDRANT_URL"]; TOPK = int(os.environ.get("TOPK", "20"))
KEY = os.environ.get("NVIDIA_API_KEY"); assert KEY, "set NVIDIA_API_KEY"
...
def e5_query(q):
    return jreq(EMB, {"model": E5_MODEL, "input": [q], "input_type": "query", "truncate": "END"},
                hdr=H)["data"][0]["embedding"]


def rerank(q, passages):
    rk = jreq(RR, {"model": RR_MODEL, "query": {"text": q},
                   "passages": [{"text": (t or " ")[:4000]} for t in passages]}, hdr=H)["rankings"]
...
def main():
    ...
    bge = BgeM3Embedder()
    A_hyb = build_hybrid_searcher(HYBRID, embedder=bge, mode="hybrid", dense_top_k=TOPK, sparse_top_k=TOPK)
    A_den = build_hybrid_searcher(HYBRID, embedder=bge, mode="dense", dense_top_k=TOPK)

    cand = {}
    t0 = time.time()
    for i, q in enumerate(queries):
        nh = A_hyb.search(q["query"]); nd = A_den.search(q["query"])
        cand[i] = {"A_hybrid": [(n.metadata.get("message_id"), n.get_content()) for n in nh],
    ...
    def c_search(i):
        ...
        hits = jreq(f"{QD}/collections/{E5}/points/search",
                    {"vector": {"name": "dense", "vector": qv}, "limit": TOPK, "with_payload": True})["result"]
    ...
    jobs = [(i, "A_hybrid") for i in range(len(queries))] + [(i, "C_dense") for i in range(len(queries))]
    t0 = time.time()
    with ThreadPoolExecutor(max_workers=3) as ex:
    ...
    for i, q in enumerate(queries):
        gold = q["answer_message_id"]; cat = q.get("category", "?")
        for a in ARMS:
            r = rank_of(cand[i][a], gold); A = agg[a]; C = bycat[a][cat]; A["n"] += 1; C["n"] += 1
            if r is not None:
                for k in (1, 5, 10):
                    if r < k:
                        A[f"r{k}"] += 1; C[f"r{k}"] += 1
                A["mrr"] += 1 / (r + 1); C["mrr"] += 1 / (r + 1)
    ...
    for a in ARMS:
        print(f"{a:20s} {pct(a,1):6.1f} {pct(a,5):6.1f} {pct(a,10):6.1f} {agg[a]['mrr']/agg[a]['n']:6.3f}")
    cats = sorted({q.get("category", "?") for q in queries})
    print("\n==== by category: R@5 ====")
    print(f"{'arm':20s} " + " ".join(f"{c:>9}" for c in cats))
    for a in ARMS:
        print(f"{a:20s} " + " ".join(
            f"{100*bycat[a][c]['r5']/max(bycat[a][c]['n'],1):9.1f}" for c in cats))
    print("\nDONE", flush=True)
