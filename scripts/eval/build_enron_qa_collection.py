...
f = [
    x
    for x in glob.glob(
        "/Users/fmasi/Git/mailrag/data_cache/MichaelR207___enron_qa_0922/**/*.arrow", recursive=True
    )
    if "test" in x
][0]
try:
...
usable = [
    r for r in rows if r.get("questions") and r["questions"] and len((r.get("email") or "")) > 120
]
random.seed(13)
...
queries = [
    {"query": r["questions"][0], "answer_message_id": r["path"], "category": "enron"}
