...
def jreq(url,body,hdr,timeout=60):
    for a in range(8):
        try:
            r=urllib.request.Request(url,data=json.dumps(body).encode(),headers=hdr,method="POST")
...
def rerank(q,passages):
    rk=jreq(RR,{"model":"nvidia/rerank-qa-mistral-4b","query":{"text":q},"passages":[{"text":(t or " ")[:4000]} for t in passages]},H)["rankings"]
...
bge=BgeM3Embedder()
S=build_hybrid_searcher("work-rag-ctx-threadaware",embedder=bge,mode="hybrid",dense_top_k=TOPK,sparse_top_k=TOPK)
...
for q in queries:
    cat=q.get("category","?"); gmid=q["answer_message_id"]; gtid=q["thread_id"]
    ...
    for a in arms:
        hits=variants[a]; er=email_rank(hits,gmid); tr=thread_rank(hits,gtid)
        for cat_key in (cat,"ALL"):
            d=agg[a][cat_key]; d["n"]+=1
            for k in (1,5,10):
                if er is not None and er<k: d[f"e{k}"]+=1
                if tr is not None and tr<k: d[f"t{k}"]+=1

cats=["ALL","terse","content","spanning"]
for a in arms:
    print(f"\n==== arm: {a}  (EMAIL-recall@k  vs  THREAD-recall@k, %) ====")
    print(f"{'category':10s} {'n':>4}  {'E@1':>5}{'T@1':>6}   {'E@5':>5}{'T@5':>6}   {'E@10':>5}{'T@10':>6}")
    for c in cats:
        d=agg[a][c]; n=d['n']
        if not n: continue
        def p(m): return 100*d[m]/n
        print(f"{c:10s} {int(n):>4}  {p('e1'):5.1f}{p('t1'):6.1f}   {p('e5'):5.1f}{p('t5'):6.1f}   {p('e10'):5.1f}{p('t10'):6.1f}")
print("\nDONE",flush=True)
