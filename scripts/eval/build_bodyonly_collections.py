...
QD=os.environ["QDRANT_URL"]; SRC="work-rag-ctx-threadaware"
...
def jreq(url,body=None,method="POST",hdr=None,timeout=120):
    for a in range(8):
        try:
            ...
            with urllib.request.urlopen(r,timeout=timeout) as z: return json.load(z) if z.length!=0 else {}
...
for i in range(0,len(bodies),256):
    bb=bodies[i:i+256]; bm=mids[i:i+256]
    ...
    hq.upsert(client,"work-rag-bge-body",points); n+=len(points)
print(f"bge-m3 body-only: {n} pts in {time.time()-t0:.0f}s",flush=True)

# --- e5 dense body-only ---
def embed(batch):
    return [d["embedding"] for d in jreq(EMB,{"model":"nvidia/nv-embedqa-e5-v5","input":batch,"input_type":"passage","truncate":"END"},hdr=H)["data"]]
...
try: jreq(f"{QD}/collections/work-rag-e5-body",method="DELETE",timeout=15)
except urllib.error.HTTPError: pass
jreq(f"{QD}/collections/work-rag-e5-body",{"vectors":{"dense":{"size":1024,"distance":"Cosine"}}},method="PUT",timeout=15)
P=[{"id":i,"vector":{"dense":vecs[i]},"payload":{"message_id":mids[i],"text":bodies[i][:6000]}} for i in range(len(vecs))]
for j in range(0,len(P),512):
    jreq(f"{QD}/collections/work-rag-e5-body/points?wait=true",{"points":P[j:j+512]},method="PUT",timeout=60)
print(f"DONE body-only collections: work-rag-bge-body, work-rag-e5-body",flush=True)
