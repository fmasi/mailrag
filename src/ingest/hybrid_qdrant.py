...
def ensure_hybrid_collection(client: QdrantClient, name: str, dim: int = 1024, recreate: bool = False) -> None:
    ...
    if not client.collection_exists(name):
        ...
        # Payload indexes so filtering is fast/allowed: recipients & sender
        # (full-text, for "emails involving person X"), subject (full-text),
        # and exact-match keys (thread = whole conversation, source, date).
        _PAYLOAD_INDEXES = [
