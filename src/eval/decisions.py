...
from src.eval.metrics import ndcg_at_k, aggregate
...
def decide(rankings: List[Dict], grades: Dict, pct_over_8k: Optional[float] = None) -> Dict[str, bool]:
    thread = aggregate(_per_query_ndcg(rankings, grades, THREAD_ARM), ["ndcg@10"])
    # Best (max) C' arm per group is the bar to clear.
    cprime_groups = [aggregate(_per_query_ndcg(rankings, grades, a), ["ndcg@10"]) for a in CPRIME_ARMS]
    ...
    content_ok = thread.get("content", {}).get("ndcg@10", 0.0) >= cprime_best("content")  # strict beat
