...
def _cleaned_token_lengths(profile):
    ...
    return [len(tok.encode(e.body, add_special_tokens=False)) for e in emails if (e.body or "").strip()]


def run(profile, *, set_profile=False) -> ProfileReport:
    lens = _cleaned_token_lengths(profile)
    suggested = suggest_chunk_size(lens)
    report = ProfileReport(percentiles=percentiles(lens),
                           mean=statistics.mean(lens) if lens else 0.0,
                           bodies=len(lens), suggested_chunk_size=suggested)
    if set_profile:
