...
from src.cluster.noise_pockets import ThreadMeta, cluster_threads, ClusterReport
from src.cluster.noise_pockets import format_report  # re-export for the CLI
...
def _is_tagged(email) -> bool:
    return bool(getattr(email, "noise_candidate", False)
                or getattr(email, "is_bulk", False))
...
def aggregate_threads(emails) -> Tuple[List[ThreadMeta], Dict[str, List[int]]]:
    ...
    for tid, idxs in tid_to_idx.items():
        ...
        for m in members:
            ...
            if len(subjects) >= 3:
                break
        metas.append(ThreadMeta(
            thread_id=tid, n_emails=len(members), dominant_sender=dominant,
...
def _fresh_thread_vectors(emails, tid_to_idx, *, batch_size=32) -> Dict[str, np.ndarray]:
    ...
    dense, _ = embedder.encode([getattr(e, "body", "") or "" for e in emails],
                               batch_size=batch_size)
    dense = np.asarray(dense, dtype=float)
    return {tid: dense[idxs].mean(axis=0) for tid, idxs in tid_to_idx.items()}


def run(profile, *, json_path: str, clusters: int | None = None, seed: int = 11,
        limit: int | None = None, force_fresh: bool = False,
        qdrant_url: str | None = None, top: int = 15,
        profile_path: str = "") -> ClusterReport:
    ...
    payload = report.to_json_dict(
        profile=profile_path, collection=profile.collection,
